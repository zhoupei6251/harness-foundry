---
artifact: spec
title: "三层 Harness 集成：harness-foundry（主） + Superpowers 补缺（辅） + ECC 专项 agent（按需）"
date: 2026-06-22
status: superseded
superseded_at: 2026-06-25
superseded_reason: "第三方目录已整合到主池，所有 skill 统一在 skills/ 扁平结构下"
platform: harness-foundry
route: superpowers:brainstorming
approved: true
related:
  - harness-foundry/core/intent-routing.md
  - harness-foundry/adapters/trae/skill-binding.md
  - harness-foundry/adapters/cursor/skill-binding.md
...
> **状态说明**：本文档描述的是 2026-06-22 的架构决策。2026-06-25 已完成第三方整合，所有 skill 统一在 `skills/` 扁平结构下，`third-party/` 目录已删除。当前架构见 [README.md](../../../README.md)。

## 1. 背景与问题

心悦 AIGC 项目（`aigc_platfrom_back`）当前使用 harness-foundry 作为 AI 编码工作流框架。用户在工作目录里同时存在两个外部开源项目：
...
**诉求：** 是否能把 Superpowers + ECC 的能力叠加到 harness-foundry 上，得到"最强的 AI 编码工作流"。

**调研发现：**

| 事实 | 影响 |
|------|------|
| harness-foundry 已从 Superpowers 同步了 80% 核心 skill（test-driven-development / systematic-debugging / brainstorming / requesting-code-review / verification-before-completion 等） | 直接装 Superpowers 整个插件会**重复加载同名 skill** |
| harness-foundry 已定义 7 个专属 agent + 完整意图路由 | ECC 的命令式工作流（`/plan` `/tdd` `/code-review`）会**抢意图路由** |
| Superpowers 缺少 4 个 skill：`subagent-driven-development` / `dispatching-parallel-agents` / `using-git-worktrees` / `executing-plans` | 这些是 SDD（Subagent-Driven Development）核心流程，harness-foundry 当前未集成 |
...
| L1 主 | harness-foundry 保持原状，已包含 Superpowers 80% 核心 |
...
1. **零冲突**：所有新增 skill/agent 不与 harness-foundry 现有命名/路径冲突。
...
5. **不破坏意图路由**：harness-foundry 的 `intent-routing.md` 优先级保持最高。

### 3.2 非目标

- 装 Superpowers / ECC 整个插件
- 修改 harness-foundry 现有 skill 内容
- 替换 harness-foundry 的 7 个核心 agent
...
| 来源 | 目录 | 命名规则 |
|------|------|---------|
| harness-foundry 原生 | `.trae/skills/<slug>/` | 直接使用 slug |
...
```
harness-foundry/third-party/        ← 真相源（git tracked）
├── superpowers/skills/<slug>/
└── ecc/agents/ecc-<name>.md

        ↓ sync-skills.sh --target trae

.trae/                            ← IDE 运行时副本（git ignored）
├── skills/<slug>/SKILL.md
└── agents/ecc-<name>.md
```

**原则：**
- `skills/` 和 `agents/` 是 source of truth（扁平化，无 third-party/ 子目录）
- `.trae/` 是 IDE 运行时副本，由 `scripts/sync-skills.sh` 投影生成
- 第三方 cherry-pick skill 由 `sync-skills.sh` 的 `SKIP_FROM_SYNC` 列表保留，不被覆盖或裁剪
- 原 `ECC/` 和 `superpowers/` 目录已**删除**（不属于项目级别）
- 升级流程：diff 上游 `_meta.json` 的 `source_version` → 手动覆盖

## 5. 防冲突与防回归机制

### 5.1 sync-skills.sh 跳过规则

修改 `harness-foundry/scripts/sync-skills.sh`，新增：
...
修改 `harness-foundry/agents/registry.md`，新增：
...
| 风险 | 等级 | 缓解措施 |
|------|------|---------|
| skill 命名冲突 | 低 | 已逐个比对，4 个 slug 在 harness-foundry 中不存在 |
...
- [ ] `harness-foundry/adapters/trae/skill-binding.md` 新增"第三方来源"章节
- [ ] `harness-foundry/agents/registry.md` 新增 ECC agent 注册
- [ ] `harness-foundry/scripts/sync-skills.sh` 新增 SKIP_FROM_SYNC 列表
- [ ] 不修改任何 harness-foundry 已有文件

...
description: Shared orchestration engine for the orch-* skill family. Defines the
  ...
  and the two human gates that the orch-* oper...
metadata:
  origin: ECC
version: 1.0.0
when_to_use: 调用 orch-pipeline 时
status: peripheral
tags:
- shared
domain: code
...
- **2. Plan** — delegate to the `planner` agent (or `architect` /
  `architect` for structural decisions). Output a `task_list` ordered as
...
|-------|---------|----------------------|
| Intake / understand | `explorer` | trace existing paths before a tweak, fix, or refactor |

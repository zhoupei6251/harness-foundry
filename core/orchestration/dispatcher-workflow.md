...
派发 WU 前执行 Input Guardrail 检查（参见 `hooks/guardrails/guardrail-config.json`）：
...
整合阶段执行 Output Guardrail（参见 `hooks/guardrails/guardrail-config.json`）：
...
- **code-reviewer**: 技术栈专项代码审查（React/Node/Java 深层模式）
- **reviewer**: 通用五轴代码审查（尾盘默认）
- **web-investigator**: 调研取证
- **debugger**: 调试修复
- **explorer**: 只读探查

#### novel 域
- **novel-writer**: 写章节正文（核心写手）
- **novel-planner**: 大纲/分卷规划、结构拆解
- **novel-reviewer**: 章节质量审查、7维评分
...
| 规划 | `writing-plans` | `brainstorming` + `junli-ai-novel` | — |
| 实现 | 平台编排 skill + Task | `novel-orchestrator` + Task | 平台编排 skill + Task |
